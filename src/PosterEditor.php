 ...
 * @link     https://github.com/wordpressvn/poster-editor
 */
...
class PosterEditor
{
    ...
    public function rectangle($x, $y, $width, $height, $options = array())
    {
        ...
        if (isset($options['gradient']) && $options['gradient'] !== '') {
            ...
            $trans = imagecolorallocatealpha($this->resource, 0, 0, 0, 127);
            imagefill($this->resource, 0, 0, $trans);
            for ($i = 0; $i < $width; ++$i) {
                $alpha = $i <= $x ? 0 : round(min(($i - $x) / ($width - $x), 1) * 127);
                $gradients = imagecolorallocatealpha($this->resource, $rgb[0], $rgb[1], $rgb[2], $alpha);
                imageline($this->resource, $i, $height, $i, 0, $gradients);
            }
            return $gradients;
        }
    ...
    protected function drawLine($line, $options, $width, $x, $y, $color, $last)
    {
        ...
        // Calculate extra space for justify alignment
        $extraspace = (int)($options['fontsize'] / 50); // Convert to int

        if (false === $last) {
            $numWords = count($words);
            if ($numWords > 1) {
                $extraspace += (int)(($options['width'] - $width) / ($numWords - 1)); // Convert to int
            }
        }
        ...
        foreach ($words as $index => $word) {
            ...
            // Update x-coord
            $x = (int)($x + abs($sizes[6] - $sizes[4]) + $extraspace); // Convert to int
        }
    }
        ...
        switch ($options['vertical']) {
            ...
            case 'justify':
                if ($bias > 1) {
                    ...
                    $y = $y + ($options['height'] - $height) / 2; // Or any other fallback behavior
                }
                break;
        }
    ...
    public function flip($direction = 'horizontal')
    {
        ...
        if ($direction !== 'horizontal' && $direction !== 'vertical') {
            throw new InvalidArgumentException('Invalid flip direction. Use "horizontal" or "vertical".');
        }

        // Flip image using GD functions.
    ...
    /**
     * Trim Text HTML to array
     *
     * @return $this
     */
    public function trim_text($html) {
        ...
        foreach ($results as $result) {
            $color = '';
            $tags = '';
            if (preg_match('/<([^ >]+)([^>]*)>([^<]*)<\/\1>/', $result, $matches)) {
                $tag = $matches[1];
                $attributes = $matches[2];
                $content = $matches[3];
                if (preg_match('/style="color:\s*([^"]+)/', $attributes, $color_match)) {
                    $color = $color_match[1];
                }
                $tags = $tag;
            } else {
     ...
     * @return $this
     */
     public function slash_text($str, $slash = null) {
        $str = stripslashes($str);
        switch ($slash) {
            case 'square':
                if (!preg_match('/^(.*?)\[(.*?)\](.*?)$/', $str, $matches)) return false;
                break;
            case 'round':
                if (!preg_match('/^(.*?)\(([^)]+)\)(.*?)$/', $str, $matches)) return false;
                break;
        ...
        return array_filter([
            isset($matches[1]) ? htmlspecialchars(trim($matches[1])) : null,
            isset($matches[2]) ? htmlspecialchars(trim($matches[2])) : null,
            isset($matches[3]) ? htmlspecialchars(trim($matches[3])) : null,
        ]);
    }
